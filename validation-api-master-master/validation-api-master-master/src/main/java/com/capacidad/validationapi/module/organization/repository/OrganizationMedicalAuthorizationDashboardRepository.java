...
@Repository
@TenantFilter
public interface OrganizationMedicalAuthorizationDashboardRepository extends JpaRepository<BaseEntity<Long>, Long> {
    ...
    @Query(value = "select case " +
            ...
            "inner join m.practitioner pr " +
            "inner join pr.medicalRegistrations mrg " +
            "inner join mrg.organization org " +
            "where m.contract in ?5 and org.resourceId = ?6 " +
            "group by key " +
            "order by key")
    List<Tuple> countAllByCreatedAtBetweenGroupedByAge
            (LocalDateTime from, LocalDateTime to, LocalDateTime prevFrom, LocalDateTime prevTo, Set<Contract> contracts, UUID resourceId);
    ...
    @Query(value = "select (case " +
            ...
            "inner join pr.medicalRegistrations mrg " +
            "inner join mrg.organization org " +
            "where (m.createdAt between ?1 and ?2) and m.contract in ?3 and org.resourceId = ?4 " +
    ...
    List<XYPoint> authorizationsGraphGroupedByGender(LocalDateTime from, LocalDateTime to, Set<Contract> contracts, UUID resourceId);
    ...
    List<XYPoint> authorizationsGraphGroupedByCity(LocalDateTime from, LocalDateTime to, Set<Contract> contracts, UUID resourceId, Pageable pageable);
    ...
    @Query(value = "select new com.capacidad.validationapi.module.dashboard.dto.XYPoint(mp.name, count(it)) " +
            ...
            "inner join pr.medicalRegistrations mrg " +
            "inner join mrg.organization org " +
            "where (it.createdAt between ?1 and ?2) and ms.id = ?3 and m.contract in ?4 and org.resourceId = ?5 " +
            "group by mp.name " +
            "order by mp.name")
    List<XYPoint> authorizationsGraphGroupedBySpecialty(LocalDateTime from, LocalDateTime to, Long specialtyId, Set<Contract> contracts, UUID resourceId);
    ...
    List<XYPoint> authorizationsGraphGroupedByDate(LocalDateTime from, LocalDateTime to, Set<Contract> contracts, UUID resourceId);
    ...
    @Query(value = "select new com.capacidad.validationapi.module.dashboard.dto.KeyValueReport(s.name, count(m)) " +
            ...
            "where (m.createdAt between ?1 and ?2) and m.contract in ?3 and org.resourceId = ?4 " +
            "group by s.name")
    List<KeyValueReport> countAllByCreatedAtBetweenGroupedByStatus(LocalDateTime from, LocalDateTime to, Set<Contract> contracts, UUID resourceId);
