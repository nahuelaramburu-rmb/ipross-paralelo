...
@Repository
@TenantFilter
public interface PractitionerMedicalAuthorizationDashboardRepository extends JpaRepository<BaseEntity<Long>, Long> {
    ...
    @Query(value = "select new com.capacidad.validationapi.module.dashboard.dto.KeyValueReport(s.name, count(m)) " +
            ...
            "where (m.createdAt between ?1 and ?2) and p.resourceId = ?3 " +
            "group by s.name")
    List<KeyValueReport> countAllByCreatedAtBetweenAndStatus
    ...
    @Query(value = "select new com.capacidad.validationapi.module.dashboard.dto.XYPoint(mp.name, count(it)) " +
            ...
            "inner join m.practitioner p " +
            "where (it.createdAt between ?1 and ?2) and ms.id = ?3 and p.resourceId = ?4 " +
            "group by mp.name " +
            "order by mp.name")
