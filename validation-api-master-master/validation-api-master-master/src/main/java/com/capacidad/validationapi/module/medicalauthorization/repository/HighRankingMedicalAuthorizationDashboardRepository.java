...
@TenantFilter
@Repository
public interface HighRankingMedicalAuthorizationDashboardRepository extends JpaRepository<BaseEntity<Long>, Long> {
    ...
    List<Tendency> countAllByCreatedAtBetweenGroupedByGender
            (LocalDateTime from, LocalDateTime to, LocalDateTime prevFrom, LocalDateTime prevTo);
    ...
    List<XYPoint> authorizationsGraphGroupedByGender(LocalDateTime from, LocalDateTime to);
    ...
    List<XYPoint> authorizationsGraphGroupedByCity(LocalDateTime from, LocalDateTime to, Pageable pageable);
    ...
    @Query(value = "select new com.capacidad.validationapi.module.dashboard.dto.XYPoint(mp.name, count(it)) " +
            ...
            "where (it.createdAt between ?1 and ?2) and ms.id = ?3 " +
            "group by mp.name " +
            "order by mp.name")
    List<XYPoint> authorizationsGraphGroupedBySpecialty(LocalDateTime from, LocalDateTime to, Long specialtyId);
    ...
    List<XYPoint> authorizationsGraphGroupedByDate(LocalDateTime from, LocalDateTime to);

    @Query(value = "select new com.capacidad.validationapi.module.dashboard.dto.Tendency(" +
            "sum(case when ma.createdAt between ?1 and ?2 then it.chargeSubtotal else 0 end), " +
            "sum(case when ma.createdAt between ?3 and ?4 then it.chargeSubtotal else 0 end)) " +
            "from MedicalAuthorizationItem it " +
            "inner join it.medicalAuthorization ma " +
            "where ma.status.id = ?5 " +
            "and ma.paymentMethod.id = ?6")
    Tendency sumAllChargesByCreatedAtBetween(LocalDateTime from,
                                             LocalDateTime to,
    ...
    @Query(value = "select new com.capacidad.validationapi.module.dashboard.dto.KeyValueReport(c.name, sum(it.chargeSubtotal)) " +
            "from MedicalAuthorizationItem it " +
            "inner join it.medicalAuthorization ma " +
            "inner join ma.company c " +
            "where ma.createdAt between ?1 and ?2 and ma.status.id = ?3 and ma.paymentMethod.id = ?4 " +
            "group by c.name " +
            "order by sum(it.chargeSubtotal) desc")
    List<KeyValueReport> sumAllChargesByCreatedAtBetweenGroupedByCompany(LocalDateTime from,
                                                                         LocalDateTime to,
                                                                         long statusId,
                                                                         long paymentMethodId);
