...
do
$do$
    declare r medical_authorization_item%rowtype;
    begin
        for r in select * from medical_authorization_item
            loop
               update settlement_item set medical_authorization_item_id = (
                        select id
                        from medical_authorization_item mit
                        where mit.medical_authorization_id = r.medical_authorization_id
                          and mit.nomenclator_id = r.nomenclator_id
                    )
               where settlement_item.medical_authorization_id = r.medical_authorization_id
               and settlement_item.nomenclator_id = r.nomenclator_id;
            end loop;
    end
$do$;
